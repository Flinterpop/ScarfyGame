...
void TileMap2D::getLayers() {
	auto& layers = ptileMap->getLayers();
	for (auto& layer : layers) {
		auto px = layer.getParallax();
		//printf("Parallax: %f  %f\r\n", px.x, px.y);
	}
...
void TileMap2D::generatePhysicsObjects(b2World &physicsWorld, float worldScale) {
	...
	auto &layers = ptileMap->getLayers();
	auto tileSizeTson = ptileMap->getTileSize();
