...
void CharacterActor::goRocketDown() {
	if (true) {
		b2Vec2 velocity = getVelocity();
		velocity.y = +walkSpeed;// -jumpSpeed;
		setVelocity(velocity);
	}
}

void CharacterActor::goRocketUp() {
	if (true) {
		b2Vec2 velocity = getVelocity();
		velocity.y = -walkSpeed;// -jumpSpeed;
		setVelocity(velocity);
...
void CharacterActor::goRocketLeft() {
		if (true) {
			b2Vec2 velocity = getVelocity();
...
void CharacterActor::goRocketRight() {
	if (true) {
		b2Vec2 velocity = getVelocity();
		velocity.x = +walkSpeed/5;
		setVelocity(velocity);
